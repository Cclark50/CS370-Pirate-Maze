...
class TreasureMaze(object):
    ...
    def update_state(self, action):
        ...
        if not valid_actions:
            ...
            if action == LEFT:
                ...
                nrow -= 1
            #bug where this next elif was just an if when the entire chain should be elifs
            elif action == RIGHT:
                ncol += 1
            elif action == DOWN:
                nrow += 1
        else:                  
            #fixed typo: previously was 'mode' instead of 'nmode'
            #this was causing the state to never be updated to 'invalid' when an invalid action was taken
    ...
    def get_reward(self):
        ...
        if pirate_row == nrows-1 and pirate_col == ncols-1:
            #increased the reward gained from winning
            #the original was 1.0 for a win, but the penalty for a step was -0.04
            #the consequence of this was that any more than 25 steps would result in a negative score even with a win
            #changing this to a reward of 8 so that even with a win that takes the whole 150 steps i've set to max
            #it will gain a positive score for a win. 
            #anything less than 6 would result in a negative for a win after 25*score steps, and 6 is a break even at 150
            return 6.0
        if mode == 'blocked':
            return self.min_reward - 1
        #changing the order of operations here
        #if the check for visited happend on a cell the AI tried to visit but was blocked, its possible that that cell could have been marked visited and would have returned .25 instead of .75
        if mode == 'invalid':
            return -0.75
        if (pirate_row, pirate_col) in self.visited:
            #changing the penalty of going backwards from -0.25 to the same as visiting a cell (-0.04)
            #this is extremely important since if we're starting from a free cell every time and the AI decides to go in the wrong direction at first
            #it will get a negative score for going in the right direction later
            return -0.25
        if mode == 'valid':
            return -0.04
